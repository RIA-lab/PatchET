...
from models.patch_layers import *
from models.RevIN import RevIN


# Cell
class PatchIntraBackbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
                 ...
                 d_ff: int = 256, norm: str = 'BatchNorm', attn_dropout: float = 0.1, dropout: float = 0.1,
                 ...
                 pretrain_head: bool = False, head_type='flatten', individual=False, revin=False, affine=True,
                 subtract_last=False,
        ...
        # RevIn default false
        self.revin = revin
        if self.revin: self.revin_layer = RevIN(patch_num, affine=affine, subtract_last=subtract_last)
    ...
    def forward(self, z):  # z: [B x L x D]
        # do patching
        if self.padding_patch == 'end':
            z = self.padding_patch_layer(z)
        z = z.unfold(dimension=1, size=self.patch_len, step=self.stride)  # z: [B x patch_num x D x patch_len]
        z = z.permute(0, 1, 3, 2)  # z: [B x patch_num x patch_len x D]
        ...
        # model
        z = self.backbone(z)  # z: [B x patch_num x D x patch_len]
        z = self.head(z)  # z: [B x patch_num x D'] where D' is target_window
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        # Input x: [B x patch_num x patch_len x D]
        n_vars = x.shape[1]


        u = torch.reshape(x, (x.shape[0] * x.shape[1], x.shape[2], x.shape[3]))  # u: [B * patch_num x patch_len x D]
        u = self.dropout(u + self.W_pos)  # u: positional encoding

        # Encoder
        z = self.encoder(u)  # z: [B * patch_num x patch_len x D]
        z = torch.reshape(z, (-1, n_vars, z.shape[-2], z.shape[-1]))  # z: [B x patch_num x patch_len x D]
        z = z.permute(0, 1, 3, 2)  # z: [B x patch_num x D x patch_len]
